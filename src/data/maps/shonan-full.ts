...
const decorations: MapDecoration[] = [
  ...
  // beach/coastline(どちらもx: -100〜1750の曲線)が届かない東西の縁を薄く「海」で下地として
  // 埋める安全網。稲村ヶ崎海岸(x:1725〜1767, y:1063〜1147)はこの曲線の東端(x:1750)を
  // わずかに超えており、他の地形装飾も無いため何も無い背景に見えていた。pos:850はbeach曲線の
  // 最北点(y:830)よりわずかに南なので、配列の後ろにあるbeach/coastlineがこの上に重なって
  // 見た目の主役はそのまま変わらず、それらが届いていない縁にだけ効く(北鎌倉などの内陸部は
  // y方向で対象外なので影響しない)。
  { kind: "sea", edge: "bottom", pos: 850 },
  ...
  // ------------------------------------------------------------------
  // Phase2「地形を道路沿いに増やす」(盤面ビジュアル監査より)。
  // 既存の6件と同じ考え方(地域と地域の間の空白・海岸沿い・川沿いにだけ寄せる)を踏襲し、
  // 均等に敷き詰めることはしない。8件すべて既存のterrain/parkBlobバリアントのみを使い、
  // ノード・エッジ・カメラ・ズーム・LOD閾値・描画スタイルには一切触れていない。
  // ------------------------------------------------------------------

  // 茅ヶ崎region(x250〜680/y577〜956)の北端。海側(beach/coastline)だけでなく内陸側にも
  // 緑を置き、既存hills(720,430)とは重ならない距離を確保。
  { kind: "terrain", variant: "forest", cx: 480, cy: 600, rx: 70, ry: 50, rotation: -6 },

  // 寒川の南東部。既存forest(220,300、寒川西側)とは役割を分け、相模川(river2)沿いの
  // 土地利用として田園地帯を追加。既存forest/hillsのどちらとも十分な距離を確保。
  { kind: "terrain", variant: "farmland", cx: 310, cy: 440, rx: 85, ry: 55, rotation: -8 },

  // 湘南台は現状texture(住宅地)以外の地形が皆無。ニュータウンらしい「計画的にまとまった
  // 公園」を1つだけ追加し、過密な市街地の印象を壊さない程度の小さめサイズに留める。
  { kind: "parkBlob", cx: 1300, cy: 90, rx: 55, ry: 35 },

  // 鎌倉は三方を山に囲まれた地形。既存hills(1300,560)・forest(1470,470、北鎌倉)と
  // 連続させ、鎌倉駅の北側にも丘陵の続きを見せる。overviewで3件が一体化して緑の面積が
  // 大きくなりすぎないよう、既存2件よりひと回り小さいサイズにしている
  // (実装前レビューでrx85/ry65→rx70/ry55に縮小)。
  { kind: "terrain", variant: "hills", cx: 1500, cy: 620, rx: 70, ry: 55, rotation: 6 },

  // 引地川(river1)沿いの緑地帯。線だけだった川に堤防・遊歩道の緑を仄めかす、細長い
  // parkBlobを上流側(寒川・湘南台寄り)と下流側(辻堂寄り)に1件ずつ。既存hills(720,430)・
  // parkBlob(辻堂海浜公園)とは重ならない位置。
  { kind: "parkBlob", cx: 750, cy: 190, rx: 35, ry: 55 },
  { kind: "parkBlob", cx: 810, cy: 700, rx: 35, ry: 60 },

  // 相模川(river2)沿い、寒川・平塚境の河川敷イメージ。既存の平塚farmland(130,850)・
  // 寒川forest(220,300)・上のP2-B(310,440)のどれとも重ならない中間点。
  { kind: "parkBlob", cx: 250, cy: 650, rx: 35, ry: 65 },

  // 江の島中央の緑(展望灯台・植物園イメージ)。配列の最後(beach/coastline/sea より後ろ)に
  // 置くことで、海の上に浮かぶ島の緑として前面に出るようにする(実装前レビューで

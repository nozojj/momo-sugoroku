import type { GameState, MapData, MapNode, MoneyEventDef, NodeType, Player } from "@/types/game";
import { getPropertyDef } from "@/data/properties";
import { drawableCardIds, getCardDef } from "@/data/cards";
import { moneyEventPool, moneyGainPool, moneyLossPool, localEventPool, drawFromPool } from "@/data/events";
 ...
 *   - 季節による金額変動: ctx.state.turn を getCalendar() に通して判定
...
export const LANDING_HANDLERS: Partial<Record<NodeType, LandingHandler>> = {
  ...
  moneyGain: (ctx) => moneyOutcome(moneyGainPool, ctx),
  moneyLoss: (ctx) => moneyOutcome(moneyLossPool, ctx),
